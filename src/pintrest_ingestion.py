...
for query in SEARCH_QUERIES:
    print(f"🔎 Collecting Pinterest pins for: {query}")
    ...
    for pin in pins:
        ...
        tags = "|".join({t.lower().lstrip("#") for t in re.findall(r"#\w+", description)})
...
df = pd.DataFrame(all_details).drop_duplicates(subset="pin_id")
df.to_csv(OUTPUT_CSV, index=False, encoding="utf-8")
print(f"Fetched {len(df)} Pinterest posts across {len(SEARCH_QUERIES)} queries.")
